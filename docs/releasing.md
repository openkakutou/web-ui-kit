...
5. `npm publish --provenance`, authenticated via the `NPM_TOKEN` repository secret. Provenance (OIDC-based supply-chain attestation) is enabled from this first publish onward, since it can't be retrofitted onto versions already on the registry.
