...
### Simulating a missing native `showModal()` in jsdom

`jsdom` implements no part of `HTMLDialogElement.prototype.showModal`/`close` at all — `typeof dialog.showModal` is `"undefined"`, not just a no-op. `src/components/dialog.ts` therefore never relies on the browser for `<wuik-dialog>`'s focus move/trap, Esc-to-close, backdrop-click-to-close, or focus-restore: all of it is the component's own JS, feature-detected (`typeof this.#dialog.showModal === "function"`) before ever calling the native method as a pure rendering enhancement. `dialog.test.ts` therefore already exercises the real, only code path — the "unsupported" branch — for free, the same shape as `detectWebglSupport()`'s jsdom coverage above. See `.vibe/decisions/019-dialog-manual-behavior-over-native-showmodal.md`.
