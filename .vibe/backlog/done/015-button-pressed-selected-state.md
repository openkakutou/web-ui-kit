...
## Acceptance Criteria
- [x] `<wuik-button>` accepts a boolean attribute (e.g. `pressed`) that applies a visually distinct, token-driven "pressed" style on top of its existing `variant`
- [x] The pressed state also sets `aria-pressed` on the underlying native button automatically, so a consumer no longer needs to set it by hand
- [x] Toggling the attribute updates the rendered state immediately (`attributeChangedCallback`, same pattern as `variant`/`disabled`)
- [x] The pressed style is visibly distinct in both light and dark themes and meets the same contrast bar already verified for the rest of the component (`.vibe/decisions/009`)
